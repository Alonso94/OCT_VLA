...
from oct_vla.policies.layerwise_attention import LayerwiseObjectAttention, packed_mha_query


def config():
    return SimpleNamespace(
        object_representation="entity_v2", object_attention_heads=2, object_entity_normalizer=None
    )
...
def test_matches_dual_attention_and_bias_is_not_duplicated():
    ...
    branch = LayerwiseObjectAttention(config(), 8)
...
def test_padding_permutation_empty_repeat_and_reload():
    ...
    restored = LayerwiseObjectAttention(config(), 8)
